...
Sage.init = function(elementNamesToInitLegacy) {
  ...
  initDocumentEventListener('sage.modal.closeAll',          Sage.modal.eventHandlerCloseAll);    // Event Detail Format: not supported
  initDocumentEventListener('sage.tabs.change',             Sage.tabs.eventHandlerChange);       // Event Detail Format: { tabsId: "", paneId: "" }
  ...
  // Initialize Select
  if ( shouldInitLegacy('select', '.sage-select') ) {
    Sage.select.init();
  }
